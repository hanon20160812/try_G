...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function createFieldset(program) {
    ...
    <optgroup label="T8（12mi）">
      ...
      <div class="form-group">
        <label for="coordinateXY-${program}">第一座標：</label>
        <input type="text" id="coordinateXY-${program}" name="coordinateXY">
      </div>
      <div class="form-group">
        <label for="coordinates-${program}">座標們：</label>
      ...
      <div class="form-group">
<label for="drillUpZ-${program}" style="background-color: #CDC1FF">鑽孔抬刀Z：</label>
<input type="number" id="drillUpZ-${program}" name="drillUpZ" placeholder="只要輸入數字即可" value="60"> 
</div>

<div class="form-group">
<label for="depthZ-${program}" style="background-color: #84C1FF">鑽孔深度Z：</label>
<input type="number" id="depthZ-${program}" name="depthZ" placeholder="只要輸入數字即可" min="-150" max="300" value="-10" oninput="validateDepthZ(this)">
</div>

<div class="form-group">
<label for="drillHeightR-${program}">鑽孔高度R：</label>
<input type="number" id="drillHeightR-${program}" name="drillHeightR" placeholder="只要輸入數字即可" value="10"> 
</div>


      <div class="form-group">
        <label for="feedSpeedF-${program}">進给速度F：</label>
...
<input type="text" id="programEndX" name="programEndX" placeholder="輸入程式結束位置X" value="(G0G90X-100.)"> 
  ...
  function generateCode() {
    ...
    dynamicFieldsets.forEach((fieldset) => {
      ...
      const drillHeightR = document.getElementById(`drillHeightR-${program}`).value;
...
code += `
(${toolRemark},<span style="background-color: #FFD09B"> ${toolT} ${correctionH}</span> S${spindleSpeed} F${feedSpeedF})
...
if (checkbox.checked) { // 如果複選框被選中
  if (!selectedPrograms.includes(value)) { // 如果程式編號未被選中
    selectedPrograms.push(value); // 添加到選中程式列表
    selectedPrograms.sort((a, b) => { // 對程式編號排序 (可移除)
  ...
  selectedPrograms = selectedPrograms.filter((program) => program !== value); // 從選中程式列表中移除
}
...
function updateFieldsets() {
  ...
  selectedPrograms.forEach((program) => {
    ...
    const spindleSpeedField = document.getElementById(`spindleSpeed-${program}`);
    ...
    const toolpositionField = document.getElementById(`toolPosition-${program}`);
    ...
    // 保存现有字段的值
    if (remarkField) existingValues[`toolRemark-${program}`] = remarkField.value;
    if (spindleSpeedField) existingValues[`spindleSpeed-${program}`] = spindleSpeedField.value;
    if (feedSpeedField) existingValues[`feedSpeedF-${program}`] = feedSpeedField.value;
    if (toolField) existingValues[`toolT-${program}`] = toolField.value; // 保留刀庫T的值   
    if (toolpositionField) existingValues[`toolPosition-${program}`] = toolpositionField.value; // 使用 toolpositionField
    if (rapidmoveRField) existingValues[`rapidMoveR-${program}`] = rapidmoveRField.value; // 使用 rapidmoveRField
    if (ishighestField) existingValues[`isHighest-${program}`] = ishighestField.value; // 使用 ishighestField
  });
  ...
  if (remarkField) remarkField.value = existingValues[`toolRemark-${program}`] || "";
  if (spindleSpeedField) spindleSpeedField.value = existingValues[`spindleSpeed-${program}`] || "";
  if (feedSpeedField) feedSpeedField.value = existingValues[`feedSpeedF-${program}`] || "";
  if (toolField) toolField.value = existingValues[`toolT-${program}`] || ""; // 填入刀庫T的值
  if (toolPositionField) toolPositionField.value = existingValues[`toolPosition-${program}`] || ""; 
  if (rapidMoveRField) rapidMoveRField.value = existingValues[`rapidMoveR-${program}`] || "";  
  if (isHighestField) isHighestField.value = existingValues[`isHighest-${program}`] || ""; 
